...
#         try :
#             data = request.data
#             total_payment = float(data.get('total_payment'))
...
def checkout(request):
    # carts = Cart.objects.filter(status = True)
        ...
        if fullname and from_email and your_message:
            try:
                ...
                send_mail(
                    ...
                    ['mengsonly09@gmail.com'],  # Recipient email
                    fail_silently=False
                )
...
def cart_count(request):
    ...
    # cart_length = Cart.objects.filter(disabled=False).count()
...
def telegram_sender(request):
    try:
        if request.method == 'POST':
            ...
            fullname = Checkout.objects.all().last().fullname
...
#
#             hc_id = Order.objects.filter(checkout__cartcheckout__cart__id=cart_id)
#
#             return JsonResponse({
#                 'cart_id' : cart_id,
#                 'hc_id' : hc_id
